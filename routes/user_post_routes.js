...
router.post("/addaddress",isLoggedIn,(req,res)=>{
  ...
  newaddress.save().then(
    () => User.findOneAndUpdate({_id:curruser._id},{$push:{myaddresses:newaddress}}).then(
      ()=>{
...
router.post("/addcomment",isLoggedIn,(req,res)=>{
  ...
  User.findOne({_id:curruser._id}).then(
    result => {
  ...
  newcomment.save().then(
    () => Products.findOneAndUpdate({_id:req.body.productid},{$push:{linkedcomments:newcomment}}).then(
      ()=>{
        console.log("Comment was pushed, though it shouldn't be visible yet.");
      }).catch(err => {
      res.status(400).send('Bad request');
    })
  ).catch(err => {
...
router.post("/addtocart",isLoggedIn,(req,res)=>{
  ...
  newcartitem.save().then(
    () => User.findOneAndUpdate({_id:curruser._id},{$push:{mycartingeneral:newcartitem}}).then(
      ()=>{
