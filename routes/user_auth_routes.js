...
router.route('/api/auth/sendotp')
	.post(async function(req,res){
		var phone=req.body.phone;//along with country code
		request.post(
        'https://control.msg91.com/api/sendotp.php?authkey='+process.env.msg91_AUTH_KEY+'&message=Your%20verification%20code%20is%20%23%23OTP%23%23&sender=LeCupido&mobile='+phone,
        { json: true},
...
router.route('/api/verifyotp')
	.post(async function(req,res){
		...
		request.post(
        'https://control.msg91.com/api/verifyRequestOTP.php?authkey='+process.env.msg91_AUTH_KEY+'&mobile='+phone+'&otp='+otp,
        { json: true},
        async function (error, response, body) {
            if(!error){
            	...
            	var user=await User.findOne({'contact.contact':phone});
	            	if(user){
	            		...
	            		res.status(400).send({message:'Phone no. not registered'});
	            	}
            	}
            	else{
